...
export default function ProviderCommissionTab() {
  const { user } = useAuth();
      ...
      const commissionsWithDetails = await Promise.all(
        response.documents.map(async (commission: any) => {
          try {
            ...
            // Fetch customer details using the same approach as the main commission page
            ...
            // Fetch device details using the same approach as the main commission page
  ...
  const handlePayCommission = async (commissionId: string, amount: number) => {
    try {
      ...
      if (typeof window !== 'undefined' && window.Razorpay) {
        ...
        script.onerror = () => {
          console.error('❌ [PROVIDER-COMMISSION-TAB] Failed to load Razorpay script');
          toast({
            title: "Error",
      ...
      console.error('❌ [PROVIDER-COMMISSION-TAB] Error processing commission payment:', error);
      toast({
        title: "Error",
  ...
  const initializeRazorpay = (orderData: any, commissionId: string, amount: number) => {
    try {
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            if (verifyData.success) {
              ...
              console.error('❌ [PROVIDER-COMMISSION-TAB] Payment verification failed:', verifyData.error);
              toast({
                title: "Error",
            ...
            console.error('❌ [PROVIDER-COMMISSION-TAB] Error verifying payment:', error);
            toast({
              title: "Error",
      ...
      console.error('❌ [PROVIDER-COMMISSION-TAB] Error initializing Razorpay:', error);
      toast({
        title: "Error",
                  ...
                  <CardContent className="p-6">
                    ...
                    {commission.status === 'pending' && (
                      <div className="flex justify-end">
                        <Button
                          onClick={() => handlePayCommission(commission.$id || '', commission.commission_amount || 0)}
